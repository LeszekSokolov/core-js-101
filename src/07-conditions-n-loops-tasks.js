...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  return `${start}${array[0]}, ${array[1]}${end}`;
}
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < charArr.length; i += 1) {
    if (pairs[charArr[i]]) {
      ...
      Object.values(pairs).includes(charArr[i]) &&
      charArr[i] !== pairs[array.pop()]
    ) {
      return false;
    }
  }
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 0; i < firstChar.length; i += 1) {
    if (arr.every((el) => el[i] === firstChar[i])) {
      totalStr += `${firstChar[i]}/`;
    } else {
      break;
    }
  }
